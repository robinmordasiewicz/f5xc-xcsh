...
describe("extractUsedFlags", () => {
	describe("basic flag extraction", () => {
		...
		it("ignores non-flag arguments", () => {
			const result = extractUsedFlags(["create", "healthcheck", "--name", "test"]);
	...
	describe("edge cases", () => {
		it("handles repeated flags", () => {
			const result = extractUsedFlags(["--public-ip", "1.2.3.4", "--public-ip", "5.6.7.8"]);
...
describe("parseCreationFlagValues", () => {
	describe("basic parsing", () => {
		it("parses --flag value pattern", () => {
			const result = parseCreationFlagValues(["--type", "http", "--name", "test"]);
	...
	describe("value handling", () => {
		...
		it("handles numeric string values", () => {
			const result = parseCreationFlagValues(["--port", "80", "--interval", "10"]);
		...
		it("handles equals sign in value", () => {
			const result = parseCreationFlagValues(["--header=Content-Type=application/json"]);
	...
	describe("edge cases", () => {
		...
		it("handles consecutive boolean flags", () => {
			const result = parseCreationFlagValues(["--use-http2", "--use-origin-server-name"]);
			expect(result.get("--use-http2")).toBe("");
...
describe("formatDefaultValue", () => {
	...
	describe("arrays", () => {
		...
		it("formats large arrays with ellipsis", () => {
			expect(formatDefaultValue(["a", "b", "c", "d"])).toBe("a, b, c, ... (4 items)");
		});
	});
