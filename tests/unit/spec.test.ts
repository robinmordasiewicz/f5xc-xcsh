...
describe("spec enhancements", () => {
  describe("buildHealthcheckResourceSpec", () => {
    ...
    it("includes oneOf groups for health check types", () => {
      ...
      expect(healthCheckGroup?.variants).toContain("udp_icmp_health_check");
    });
    ...
    it("includes interval field with recommended value", () => {
      ...
      const intervalField = spec.fields.find((f) => f.name === "interval");
    ...
    it("includes jitter_percent with server default", () => {
      ...
      const jitterField = spec.fields.find((f) => f.name === "jitter_percent");
    ...
    it("includes minimum configuration", () => {
      ...
      expect(spec.minimumConfiguration?.requiredFields).toBeInstanceOf(Array);
      expect(spec.minimumConfiguration?.requiredFields.length).toBeGreaterThan(
        0,
      );
      expect(spec.minimumConfiguration?.mutuallyExclusiveGroups).toBeInstanceOf(
        Array,
      );
    });
  ...
  describe("getCommandSpec", () => {
    ...
    it("handles generic resource create command pattern", () => {
      ...
      expect(
        spec1 === null || typeof spec1 === "object" || spec1 === undefined,
      ).toBe(true);
      expect(
        spec2 === null || typeof spec2 === "object" || spec2 === undefined,
      ).toBe(true);
    });
  });
