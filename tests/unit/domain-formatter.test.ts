...
describe("formatDomainOutput", () => {
  ...
  it("returns empty array for none format", () => {
    const result = formatDomainOutput(testData, {
      ...baseOptions,
      format: "none",
    });
...
describe("formatKeyValueOutput", () => {
  ...
  const baseOptions: DomainFormatOptions = {
    format: "table",
    noColor: true,
    title: "Test",
  };
  ...
  it("returns empty array for none format", () => {
    const result = formatKeyValueOutput(testData, {
      ...baseOptions,
      format: "none",
    });
    expect(result).toEqual([]);
  ...
  it("handles null and undefined values in JSON format", () => {
    ...
    const result = formatKeyValueOutput(dataWithNulls, {
      ...baseOptions,
      format: "json",
    });
    const json = result.join("\n");
...
describe("formatListOutput", () => {
  ...
  it("returns empty array for none format", () => {
    const result = formatListOutput(testData, {
      ...baseOptions,
      format: "none",
    });
    expect(result).toEqual([]);
  ...
  it("handles null values in objects", () => {
    ...
    const result = formatListOutput(dataWithNull, {
      ...baseOptions,
      format: "tsv",
    });
  ...
  it("stringifies nested objects in TSV format", () => {
    ...
    const result = formatListOutput(nestedData, {
      ...baseOptions,
      format: "tsv",
    });
