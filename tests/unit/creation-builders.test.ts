...
describe("Healthcheck Builder - buildHealthcheckRequest", () => {
  ...
  it("rejects more than 16 request headers to remove (validation at parse time)", () => {
    ...
    expect(
      parsed.errors.some((e) => e.includes("can only be used 16 times")),
    ).toBe(true);
  });
  ...
  it("throws on invalid hex in expected-response for TCP healthcheck", () => {
    const args = [
      ...
      "5",
      "--expected-response",
      "0xZZZZ", // 0x prefix with invalid hex chars
    ];
    const parsed = parseCreationFlags(args, "healthcheck");
...
describe("Healthcheck Builder - validateHealthcheckFlags", () => {
  ...
  it("validates path starts with /", () => {
    ...
    expect(validation.valid).toBe(false);
    expect(
      validation.errors.some((e) => e.includes("--path must start with /")),
    ).toBe(true);
  });
  ...
  it("accepts neither host header flag (API applies default)", () => {
    const args = ["--name", "test-hc", "--type", "http"];
    const parsed = parseCreationFlags(args, "healthcheck");
    const validation = validateHealthcheckFlags(parsed);
...
describe("UDP-ICMP Healthcheck Tests", () => {
  it("builds basic UDP-ICMP healthcheck", () => {
    const args = ["--name", "test-icmp", "--type", "udp-icmp"];
    const parsed = parseCreationFlags(args, "healthcheck");
    const request = buildHealthcheckRequest(parsed, "default");
...
describe("Origin Pool Builder - buildOriginPoolRequest", () => {
  ...
  it("builds origin pool with public names", () => {
    ...
    expect(request.spec.origin_servers[0].public_name?.dns_name).toBe(
      "api.example.com",
    );
    expect(request.spec.use_tls).toBeDefined();
  });
  ...
  it("builds origin pool with private IPs and site", () => {
    ...
    expect(
      request.spec.origin_servers[0].private_ip?.site_locator?.site.name,
    ).toBe("my-site");
  });
});
...
describe("Origin Pool Builder - validateOriginPoolFlags", () => {
  ...
  it("rejects missing name", () => {
    const args = ["--port", "80", "--public-ip", "1.2.3.4"];
    const parsed = parseCreationFlags(args, "origin_pool");
    const validation = validateOriginPoolFlags(parsed);
  ...
  it("rejects missing origin servers", () => {
    const args = ["--name", "test-pool", "--port", "80"];
    const parsed = parseCreationFlags(args, "origin_pool");
    const validation = validateOriginPoolFlags(parsed);

    expect(validation.valid).toBe(false);
    expect(validation.errors.some((e) => e.includes("origin server"))).toBe(
      true,
    );
  });
  ...
  it("requires site for private origins", () => {
    const args = ["--name", "test-pool", "--private-ip", "10.0.0.1"];
    const parsed = parseCreationFlags(args, "origin_pool");
    const validation = validateOriginPoolFlags(parsed);

    expect(validation.valid).toBe(false);
    expect(
      validation.errors.some((e) => e.includes("--site is required")),
    ).toBe(true);
  });
...
describe("Origin Pool Builder - Advanced Features", () => {
  ...
  describe("CBIP Service Origins", () => {
    it("builds origin pool with cbip-service origin", () => {
      ...
      expect(result.spec.origin_servers[0].cbip_service?.service_name).toBe(
        "my-service",
      );
      expect(
        result.spec.origin_servers[0].cbip_service?.site_locator,
      ).toBeDefined();
    });
  ...
  describe("Circuit Breaker", () => {
    ...
    it("configures custom circuit breaker", () => {
      ...
      expect(
        result.spec.advanced_options?.circuit_breaker?.connection_limit,
      ).toBe(1000);
      expect(result.spec.advanced_options?.circuit_breaker?.max_requests).toBe(
        500,
      );
      expect(result.spec.advanced_options?.circuit_breaker?.priority).toBe(
        "HIGH",
      );
    });
    ...
    it("validates child flags require parent mode", () => {
      ...
      expect(validation.errors[0]).toContain(
        "require --circuit-breaker=custom",
      );
    });
    ...
    it("validates circuit breaker connection limit range", () => {
      ...
      expect(validation.valid).toBe(false);
      expect(validation.errors).toContain(
        "--cb-connection-limit must be between 0 and 32768",
      );
    });
  ...
  describe("Outlier Detection", () => {
    ...
    it("configures custom outlier detection", () => {
      ...
      expect(
        result.spec.advanced_options?.outlier_detection?.consecutive_5xx,
      ).toBe(5);
      expect(
        result.spec.advanced_options?.outlier_detection?.max_ejection_percent,
      ).toBe(50);
    });
    ...
    it("validates child flags require parent mode", () => {
      ...
      expect(validation.valid).toBe(false);
      expect(validation.errors[0]).toContain(
        "require --outlier-detection=custom",
      );
    });
    ...
    it("validates consecutive 5xx range", () => {
      ...
      expect(validation.valid).toBe(false);
      expect(validation.errors).toContain(
        "--od-consecutive-5xx must be between 0 and 1024",
      );
    });
    ...
    it("validates max ejection percent range", () => {
      ...
      expect(validation.valid).toBe(false);
      expect(validation.errors).toContain(
        "--od-max-ejection-percent must be between 0 and 100",
      );
    });
  });
  ...
  describe("HTTP Protocol", () => {
    ...
    it("configures HTTP/1.1 header transformation", () => {
      ...
      expect(
        result.spec.advanced_options?.http1_config?.header_transformation,
      ).toBe("proper_case_header_transformation");
    });
  ...
  describe("Source IP Persistence", () => {
    it("disables source IP persistence", () => {
      ...
      expect(
        result.spec.advanced_options?.disable_lb_source_ip_persistance,
      ).toEqual({});
    });
    ...
    it("enables source IP persistence", () => {
      ...
      expect(
        result.spec.advanced_options?.enable_lb_source_ip_persistance,
      ).toEqual({});
    });
  });
  ...
  describe("Subset Load Balancing", () => {
    ...
    it("enables subsets with keys", () => {
      ...
      expect(
        result.spec.advanced_options?.enable_subsets?.endpoint_subsets,
      ).toBeDefined();
    });
    ...
    it("configures subset fallback policy", () => {
      ...
      expect(
        result.spec.advanced_options?.enable_subsets?.default_subset,
      ).toBeDefined();
      expect(
        result.spec.advanced_options?.enable_subsets?.default_subset
    ...
    it("validates child flags require parent mode", () => {
      ...
      expect(validation.valid).toBe(false);
      expect(validation.errors[0]).toContain(
        "--subset-keys requires --subset-lb=enable",
      );
    });
    ...
    it("handles multiple subset key groups", () => {
      ...
      expect(
        (
          result.spec.advanced_options?.enable_subsets?.endpoint_subsets as any
        )?.[0]?.keys,
      ).toHaveLength(2);
      expect(
        (
