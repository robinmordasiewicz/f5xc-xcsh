...
describe("HistoryManager - File Persistence", () => {
  const testPath = join(
    tmpdir(),
    `xcsh-test-history-${Date.now()}-${Math.random().toString(36).slice(2)}`,
  );
  ...
  it("should round-trip correctly", async () => {
    ...
    expect(manager3.getHistory()).toEqual([
      "round1",
      "round2",
      "round3",
      "round4",
...
describe("Sensitive Data Redaction", () => {
  describe("redactSensitive function", () => {
    it("should redact --token values", () => {
      expect(redactSensitive("login --token abc123xyz")).toBe(
        "login --token ******",
      );
    });
    ...
    it("should redact --api-token values", () => {
      expect(redactSensitive("configure --api-token mysecret")).toBe(
        "configure --api-token ******",
      );
    });
    ...
    it("should redact --password values", () => {
      expect(redactSensitive("login --password supersecret")).toBe(
        "login --password ******",
      );
    });

    it("should redact --secret values", () => {
      expect(redactSensitive("configure --secret mykey")).toBe(
        "configure --secret ******",
      );
    });

    it("should redact --certificate values", () => {
      expect(redactSensitive("configure --certificate /path/to/cert.pem")).toBe(
        "configure --certificate ******",
      );
    });

    it("should redact --cert values", () => {
      expect(redactSensitive("configure --cert /path/to/cert.pem")).toBe(
        "configure --cert ******",
      );
    });

    it("should redact --private-key values", () => {
      expect(redactSensitive("configure --private-key keydata")).toBe(
        "configure --private-key ******",
      );
    });

    it("should redact --api-key values", () => {
      expect(redactSensitive("configure --api-key myapikey")).toBe(
        "configure --api-key ******",
      );
    });
    ...
    it("should redact Authorization: Bearer headers", () => {
      expect(redactSensitive('curl -H "Authorization: Bearer xyz789"')).toBe(
        'curl -H "Authorization: Bearer ******"',
      );
    });

    it("should redact Authorization: APIToken headers", () => {
      expect(redactSensitive('curl -H "Authorization: APIToken abc123"')).toBe(
        'curl -H "Authorization: APIToken ******"',
      );
    });

    it("should redact Authorization: Basic headers", () => {
      expect(
        redactSensitive('curl -H "Authorization: Basic dXNlcjpwYXNz"'),
      ).toBe('curl -H "Authorization: Basic ******"');
    });
    ...
    it("should redact environment variable assignments", () => {
      expect(redactSensitive("export API_TOKEN=abc123")).toBe(
        "export API_TOKEN=******",
      );
      expect(redactSensitive("export F5XC_API_TOKEN=xyz789")).toBe(
        "export F5XC_API_TOKEN=******",
      );
      expect(redactSensitive("export PASSWORD=secret")).toBe(
        "export PASSWORD=******",
      );
    });

    it("should redact multiple sensitive values", () => {
      expect(redactSensitive("cmd --token abc --password xyz")).toBe(
        "cmd --token ****** --password ******",
      );
    });

    it("should not redact non-sensitive flags", () => {
      expect(redactSensitive("list --namespace default --format json")).toBe(
        "list --namespace default --format json",
      );
    });

    it("should not redact non-sensitive content", () => {
      expect(redactSensitive("help")).toBe("help");
      expect(redactSensitive("http_loadbalancer list")).toBe(
        "http_loadbalancer list",
      );
    });

    it("should be case insensitive for flag names", () => {
      expect(redactSensitive("login --TOKEN abc123")).toBe(
        "login --TOKEN ******",
      );
      expect(redactSensitive("login --Password secret")).toBe(
        "login --Password ******",
      );
    });
  });
