...
describe("Flag Parser - hasCreationFlagsInArgs", () => {
  ...
  it("detects origin_pool creation flags", () => {
    expect(
      hasCreationFlagsInArgs(
        ["--name", "test-pool", "--public-ip", "1.2.3.4"],
        "origin_pool",
      ),
    ).toBe(true);
  });
  ...
  it("returns false for unknown resource type", () => {
    expect(hasCreationFlagsInArgs(["--name", "test"], "unknown_type")).toBe(
      false,
    );
  });
  ...
  it("returns false when only non-creation flags present", () => {
    expect(hasCreationFlagsInArgs(["--output", "json"], "healthcheck")).toBe(
      false,
    );
  });

  it("handles --flag=value format", () => {
    expect(hasCreationFlagsInArgs(["--name=test-hc"], "healthcheck")).toBe(
      true,
    );
  });
});
...
describe("Flag Parser - parseCreationFlags for healthcheck", () => {
  ...
  it("validates enum values", () => {
    ...
    expect(
      parsed.errors.some((e) => e.includes("Invalid value 'invalid-type'")),
    ).toBe(true);
  });
...
describe("Flag Parser - parseCreationFlags for origin_pool", () => {
  ...
  it("rejects invalid algorithm", () => {
    ...
    expect(
      parsed.errors.some((e) => e.includes("Invalid value 'INVALID_ALGO'")),
    ).toBe(true);
  });

  it("handles boolean flags", () => {
    const args = ["--name", "test-pool", "--public-ip", "1.2.3.4", "--no-tls"];
    const parsed = parseCreationFlags(args, "origin_pool");
...
describe("Flag Parser - Edge Cases", () => {
  ...
  it("validates integer values", () => {
    ...
    expect(parsed.errors.some((e) => e.includes("Invalid integer value"))).toBe(
      true,
    );
  });

  it("prevents non-repeatable flag duplication", () => {
    const args = ["--name", "test-hc", "--name", "another-name"];
    const parsed = parseCreationFlags(args, "healthcheck");

    expect(
      parsed.errors.some((e) =>
        e.includes("cannot be specified multiple times"),
      ),
    ).toBe(true);
  });
});
