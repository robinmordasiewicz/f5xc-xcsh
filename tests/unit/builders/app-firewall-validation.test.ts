...
describe('App Firewall Validation - validateAppFirewallFlags', () => {
	describe('Required Fields', () => {
		...
		it('rejects invalid name formats', () => {
			...
			for (const name of invalidNames) {
				...
				expect(validation.valid).toBe(false);
				expect(validation.errors.some(e => e.includes('lowercase alphanumeric'))).toBe(true);
			}
		});
	});
	...
	describe('Blocking Mode Validation', () => {
		...
		it('rejects invalid blocking mode', () => {
			...
			expect(validation.valid).toBe(false);
			expect(validation.errors.some(e => e.includes('--blocking-mode'))).toBe(true);
			expect(validation.errors.some(e => e.includes('MONITORING, BLOCKING'))).toBe(true);
		});
	...
	describe('Detection Mode Validation', () => {
		...
		it('rejects invalid detection mode', () => {
			...
			expect(validation.valid).toBe(false);
			expect(validation.errors.some(e => e.includes('--detection-mode'))).toBe(true);
			expect(validation.errors.some(e => e.includes('LOW, MEDIUM, HIGH, CUSTOM'))).toBe(true);
		});
	});
	...
	describe('Request Size Validation', () => {
		...
		it('rejects size below minimum (0 bytes)', () => {
			...
			expect(validation.valid).toBe(false);
			expect(validation.errors.some(e => e.includes('between 1 and 10485760'))).toBe(true);
		});
	...
	describe('Response Codes Validation', () => {
		...
		it('accepts multiple valid response codes', () => {
			const args = ['--name', 'test', '--allowed-response-codes', '200,201,204,301-303'];
			const parsed = parseCreationFlags(args, 'app_firewall');
			const validation = validateAppFirewallFlags(parsed);
		...
		it('rejects response code below 100', () => {
			...
			expect(validation.valid).toBe(false);
			expect(validation.errors.some(e => e.includes('between 100 and 599'))).toBe(true);
		});
	...
	describe('Multiple Validation Errors', () => {
		it('reports all validation errors', () => {
			const args = [
				'--blocking-mode', 'INVALID',
				'--detection-mode', 'WRONG',
			...
			expect(validation.errors.some(e => e.includes('--blocking-mode'))).toBe(true);
			expect(validation.errors.some(e => e.includes('--detection-mode'))).toBe(true);
			expect(validation.errors.some(e => e.includes('--max-request-size'))).toBe(true);
		});
	});
	...
	describe('Edge Cases', () => {
		...
		it('handles whitespace in response codes', () => {
			const args = ['--name', 'test', '--allowed-response-codes', ' 200 , 201 '];
			const parsed = parseCreationFlags(args, 'app_firewall');
			const validation = validateAppFirewallFlags(parsed);
