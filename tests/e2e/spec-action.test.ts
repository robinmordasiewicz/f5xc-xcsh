...
describe("spec action E2E", () => {
  ...
  describe("backward compatibility", () => {
    it("should maintain backward compatibility with --spec flag", () => {
      const { stdout: oldPattern } = runCLI(
        "virtual create healthcheck --spec",
      );
    ...
    it("should work with different resources using both patterns", () => {
      ...
      const { stdout: oldAppFirewall } = runCLI(
        "virtual create app_firewall --spec",
      );
      ...
      expect(oldSpec.resourceSpec.resourceType).toBe(
        newSpec.resourceSpec.resourceType,
      );
