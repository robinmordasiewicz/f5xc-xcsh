      ...
      describe(cmd.path, () => {
        ...
        it("should acknowledge required positional arguments", async () => {
          ...
          if (onlyFlags && completionSuggestions.length > 0) {
            ...
            expect(onlyFlags, `Warning: ${errorMessage}`).toBe(false);
          }
        });
    ...
    it("all commands should have consistent path order between registry and error messages", () => {
      ...
      testableCommands.forEach((cmd) => {
        ...
        for (const errorUsage of errorUsageStrings) {
          // Validate path order
          const commandPathRegex = new RegExp(
            `^${cmd.path.replace(/\s+/g, "\\s+")}\\s+`,
          );
          ...
          const usageMatch = errorUsage.match(
            /^Usage:\s+(.+?)(?:\s+<|\s+\[|\s*$)/,
          );
          if (usageMatch) {
            ...
            if (
              extractedPath !== cmd.path &&
              !extractedPath.startsWith(cmd.path)
            ) {
              pathOrderIssues.push({
                command: cmd.path,
                registryPath: cmd.path,
    ...
    it("generateUsageError calls should use correct registry paths", () => {
      ...
      testableCommands.forEach((cmd) => {
        ...
        try {
          ...
          const generateUsageErrorPattern =
            /generateUsageError\s*\(\s*["']([^"']+)["']/g;
          let match;
      ...
      testableCommands.forEach((cmd) => {
        ...
        for (const errorUsage of errorUsageStrings) {
          ...
          const cleanedError = errorUsage
            .replace(new RegExp(`^${cmd.path.replace(/\s+/g, "\\s+")}\\s*`), "")
            .trim();
