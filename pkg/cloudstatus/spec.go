...
func getAIHints() AIHints {
	return AIHints{
		DiscoveryCommand:       "vesctl cloudstatus --spec --output-format json",
		QuickStatusCheck:       "vesctl cloudstatus status",
		ComprehensiveStatus:    "vesctl cloudstatus summary --output-format json",
		MonitoringSetup:        "vesctl cloudstatus watch --interval 60",
		AuthenticationRequired: false,
		RecommendedPolling:     60,
		BestPractices: []string{
			"Use 'vesctl cloudstatus status --quiet' in CI/CD to get exit codes (0=healthy)",
			"Use 'vesctl cloudstatus summary --output-format json' for complete state",
			"Use 'vesctl cloudstatus watch' for continuous monitoring",
			"Filter by region with 'vesctl cloudstatus pops list --region <region>'",
	...
	return []CommandSpec{
		{
			...
			Examples: []string{
				...
				"vesctl cloudstatus status --quiet && echo 'All systems operational'",
			},
		},
		{
			Name:        "summary",
	...
	return []WorkflowSpec{
		{
			...
			Steps: []WorkflowStep{
				{Step: 1, Command: "vesctl cloudstatus status --quiet", Description: "Check overall status"},
				{Step: 2, Command: "[ $? -eq 0 ] || exit 1", Description: "Exit if not healthy"},
				{Step: 3, Command: "vesctl cloudstatus incidents active --output-format json | jq '. | length'", Description: "Count active incidents"},
		...
		},
		{
			...
			Steps: []WorkflowStep{
				{Step: 1, Command: "vesctl cloudstatus status", Description: "Quick status check"},
				{Step: 2, Command: "vesctl cloudstatus incidents active", Description: "View active incidents"},
				{Step: 3, Command: "vesctl cloudstatus incidents get <id>", Description: "Get incident details"},
				{Step: 4, Command: "vesctl cloudstatus incidents updates <id>", Description: "View incident timeline"},
			},
		},
		{
			...
			Steps: []WorkflowStep{
				{Step: 1, Command: "vesctl cloudstatus pops status", Description: "Overview of all regions"},
				{Step: 2, Command: "vesctl cloudstatus pops list --region north-america", Description: "List North America PoPs"},
				{Step: 3, Command: "vesctl cloudstatus components list --pop --degraded-only", Description: "Find degraded PoPs"},
			},
		},
		{
			...
			Steps: []WorkflowStep{
				{Step: 1, Command: "vesctl cloudstatus watch --interval 30", Description: "Start monitoring"},
				{Step: 2, Command: "vesctl cloudstatus watch --exit-on-change && alert", Description: "Alert on change"},
			},
		},
	}
}
