...
function discoverResourcesFromSpec(spec: OpenAPISpec): DiscoveredResource[] {
	...
	for (const [pathPattern, pathItem] of Object.entries(spec.paths)) {
		...
		const hasNameParam =
			pathPattern.includes("{name}") ||
			pathPattern.includes("{metadata.name}");
...
async function discoverAllDomains(): Promise<DomainDiscovery[]> {
	...
	for (const specInfo of specsIndex.specifications) {
		...
		console.log(`✅ ${specInfo.domain}: ${discoveredResources.length} resources discovered from ${Object.keys(spec.paths).length} paths`);
	}
...
async function main() {
	...
	try {
		...
		console.log(`   Discovery Coverage: ${totalDiscovered - totalPrimary} additional resources`);
