...
function generateDomainEntry(domain: DomainInfo): string {
  ...
  if (domain.validationWarnings && domain.validationWarnings.length > 0) {
    const warnings = domain.validationWarnings
      .map((w) => `"${escapeString(w)}"`)
      .join(",\n\t\t\t");
    code += `\t\tvalidationWarnings: [\n\t\t\t${warnings}\n\t\t],\n`;
  }

  if (domain.resourceCategories) {
    const crudList = domain.resourceCategories.crud
      .map((r) => `"${escapeString(r)}"`)
      .join(", ");
    const analyticsList = domain.resourceCategories.analytics
      .map((r) => `"${escapeString(r)}"`)
      .join(", ");
    const utilitiesList = domain.resourceCategories.utilities
      .map((r) => `"${escapeString(r)}"`)
      .join(", ");
    const managementList = domain.resourceCategories.management
      .map((r) => `"${escapeString(r)}"`)
      .join(", ");
...
async function main(): Promise<void> {
  ...
  for (const spec of specIndex.specifications) {
    ...
    const primaryResources: ResourceMetadata[] | undefined = spec[
    ...
    if (discovery) {
      ...
      allResources = discovery.discoveredResources.map((discovered) => {
        ...
        return {
          ...
          description:
            primaryMatch?.description || discovered.description || "",
