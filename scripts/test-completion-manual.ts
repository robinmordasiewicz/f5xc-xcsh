...
async function runTests() {
  ...
  try {
    ...
    logTest(
      1,
      "/virtual create<TAB> → should show resources, not verbs (PRIMARY FIX)",
    );
    {
    ...
    logTest(5, "/certificates create<TAB> → should show certificate resources");
    {
      ...
      if (hasResources && !hasVerbs) {
        logPass(
          "✓ Shows certificate resources: " + texts.slice(0, 5).join(", "),
        );
        results.push({
    ...
    logTest(
      7,
      "/virtual<TAB> → should still show verbs initially (REGRESSION TEST)",
    );
    {
    ...
    log(
      `Passed: ${passedCount}`,
      passedCount === results.length ? GREEN : RESET,
    );
    if (failedCount > 0) {
    ...
    if (passedCount === results.length) {
      log(`\n${BOLD}${GREEN}✓✓✓ All manual tests passed! ✓✓✓${RESET}`, GREEN);
      log(
        `${GREEN}The RESOURCE_ACTIONS fix is working correctly.${RESET}`,
        GREEN,
      );
      log(
        `${GREEN}No verbs are shown after verb selection - only resources.${RESET}\n`,
        GREEN,
      );
      process.exit(0);
    } else {
      log(`\n${BOLD}${RED}✗ Some tests failed${RESET}`, RED);
