...
const testCases = [
	...
	{ label: "http_loadbalancer", desc: "HTTP/HTTPS load balancer with origin pools and routing rules" },
	{ label: "calls_by_response_code", desc: "API call statistics grouped by response code" },
];
...
console.log(`  Unselected "   ": ${stringWidth(unselectedIndicator)} visual chars`);
console.log("");
...
console.log(`  ${stringWidth(boxLeft)} (left) + ${stringWidth(selectedIndicator)} (indicator) + ${maxLabelWidth} (label) + ${stringWidth(separator)} (sep) + ${stringWidth(boxRight)} (right)`);
console.log(`  = ${totalDecorations} chars\n`);

// 7. Available width for description
const availableForDesc = terminalWidth - totalDecorations;
console.log(`Available for description: ${terminalWidth} - ${totalDecorations} = ${availableForDesc} chars\n`);
...
testCases.forEach((tc, i) => {
	...
	// Simulate full line
	const fullLine = boxLeft + selectedIndicator + paddedLabel + separator + tc.desc + boxRight;
	const fullLineWidth = stringWidth(fullLine);

	console.log(`${i + 1}. ${tc.label}`);
	console.log(`   Description: "${tc.desc.substring(0, 50)}${tc.desc.length > 50 ? '...' : ''}"`);
	...
	console.log(`   Fits in terminal (${terminalWidth})? ${fullLineWidth <= terminalWidth ? '✅ YES' : `❌ NO - overflow by ${fullLineWidth - terminalWidth}`}`);
	console.log("");
});
...
console.log(`This gives total line width: ${totalDecorations + suggestedMaxDesc} chars (${terminalWidth - totalDecorations - suggestedMaxDesc} char margin)`);
