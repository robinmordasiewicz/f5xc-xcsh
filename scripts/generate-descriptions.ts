...
function checkCrossTierRepetition(desc: DescriptionTiers): string[] {
  ...
  const shortMediumOverlap = [...shortWords].filter((w) => mediumWords.has(w));
...
function getCliRootContext(): DomainContext {
  ...
  return {
    name: "xcsh",
    type: "cli_root",
    existingDescription: "Interactive CLI for F5 Distributed Cloud services.",
    features: [
