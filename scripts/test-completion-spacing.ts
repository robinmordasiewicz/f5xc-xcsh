...
console.log('Primary Resources (should have clean descriptions with no double spaces):\n');
...
if (virtualDomain?.primaryResources) {
  for (const resource of virtualDomain.primaryResources.slice(0, 4)) {
    ...
    console.log(`${status} ${resource.name.padEnd(20)} - "${description}" [isPrimary: true]`);
...
if (virtualDomain?.allResources) {
  ...
  for (const resource of discovered) {
    ...
    console.log(`${status} ${resource.name.padEnd(20)} - "${description}" [isPrimary: false]`);
