...
function analyzeDomainDescriptions(index: SpecIndex): DomainGap[] {
  ...
  for (const entry of index.specifications) {
    ...
    if (entry.description_short.length > 60) {
      issues.push(
        `Short description exceeds 60 chars (${entry.description_short.length})`,
      );
    }
    if (entry.description_medium.length > 150) {
      issues.push(
        `Medium description exceeds 150 chars (${entry.description_medium.length})`,
      );
    }
...
function generateReport(
  ...
  const mediumSeverity = domainGaps.filter(
    (g) => g.severity === "medium",
  ).length;
  ...
  if (operationGaps.length > 0) {
    ...
    for (const [domain, gaps] of sortedDomains) {
      ...
      for (const gap of gaps.slice(0, 10)) {
        ...
        lines.push(
          `| \`${gap.path.slice(0, 50)}...\` | ${gap.method.toUpperCase()} | ${issuesSummary} |`,
        );
      }
  ...
  lines.push(
    "2. Add `x-f5xc-operation-metadata.purpose` to operations missing it",
  );
...
async function main(): Promise<void> {
  console.log("🔍 Analyzing description gaps in upstream specs...");
  ...
  console.log(`✓ Loaded spec index v${index.version}`);
  ...
  if (domainGaps.filter((g) => g.severity === "high").length > 0) {
    console.log("");
    console.log(
      "⚠️  High severity gaps found - consider creating upstream issues",
    );
  }
