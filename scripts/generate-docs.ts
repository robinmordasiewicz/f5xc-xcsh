 ...
 */
function generateResourceReference(resources: SpecPrimaryResource[]): string {
  if (resources.length === 0) return "";

  let content = "## Resource Reference\n\n";
  content += "| Resource | Description | Tier | Dependencies |\n";
  content += "|----------|-------------|------|-------------|\n";
...
function generateRelatedSection(
  ...
  const validDomains = relatedDomains.filter((domain) =>
  ...
  for (const related of validDomains) {
    ...
    const desc =
      relatedSpec?.["x-f5xc-description-short"] ||
