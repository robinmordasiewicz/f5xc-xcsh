...
async function main() {
	console.log(`\n${BOLD}${BLUE}═══════════════════════════════════════════════════════════════${RESET}`);
	console.log(`${BOLD}${BLUE}   Manual Test: x-f5xc-conflicts-with Integration${RESET}`);
	...
	console.log(`   Total conflict relationships: ${CONFLICT_RELATIONSHIP_COUNT}\n`);
	...
	for (const [prop1, prop2] of keyConflicts) {
		...
		console.log(`   ${status} ${prop1} ↔ ${prop2} (bidirectional: ${bidirectional})`);
	}
	...
	const testCases: TestCase[] = [
		// ==================== HEALTHCHECK TESTS ====================
		{
			...
			expectAbsent: [],
			description: "Both --host-header and --use-origin-server-name should appear before either is used",
		},
		{
			name: "HC-02: After --host-header is used",
			input: "/virtual create healthcheck --type http --host-header example.com ",
			expectPresent: ["--path", "--use-http2", "--expected-status-codes"],
			expectAbsent: ["--use-origin-server-name"],
			description: "--use-origin-server-name should be hidden (conflicts with --host-header)",
		},
		{
			name: "HC-03: After --use-origin-server-name is used",
			input: "/virtual create healthcheck --type http --use-origin-server-name ",
			expectPresent: ["--path", "--use-http2", "--expected-status-codes"],
			expectAbsent: ["--host-header"],
			description: "--host-header should be hidden (conflicts with --use-origin-server-name)",
		},
		{
			name: "HC-04: Non-conflicting flags should remain",
			input: "/virtual create healthcheck --type http --path /health ",
			expectPresent: ["--host-header", "--use-origin-server-name", "--use-http2"],
			expectAbsent: [],
		...
		// ==================== ORIGIN POOL TESTS ====================
		{
			...
			expectPresent: ["--public-ip", "--public-name", "--private-ip", "--private-name"],
			expectAbsent: [],
		...
		},
		{
			...
			expectAbsent: ["--public-name", "--private-ip", "--private-name", "--k8s-service"],
		...
		},
		{
			...
			input: "/virtual create healthcheck --type http --host-header test.com --path /health --use-http2 ",
			expectPresent: ["--expected-status-codes"],
			expectAbsent: ["--use-origin-server-name", "--host-header", "--path", "--use-http2"],
		...
		},
		{
			...
			expectPresent: ["--namespace", "--output"],
			expectAbsent: [],
			description: "Global flags (--namespace, --output) should always be available",
		},
		{
			...
			expectAbsent: ["--host-header", "--use-origin-server-name", "--path"],
			description: "Type-specific flags should not appear until --type is specified",
		},
		{
			name: "EDGE-06: Origin pool with multiple public-ip (repeatable)",
			input: "/virtual create origin_pool --public-ip 1.1.1.1 --public-ip 2.2.2.2 ",
		...
		},
		{
			...
			description: "Flag matching should be case-sensitive (flags are lowercase)",
		},
		{
			name: "EDGE-08: Flag with value containing dashes",
			input: "/virtual create healthcheck --type http --host-header my-test-host.com ",
			expectPresent: ["--path"],
			expectAbsent: ["--use-origin-server-name"],
	...
	console.log(`\n${BOLD}${BLUE}═══════════════════════════════════════════════════════════════${RESET}`);
	console.log(`${BOLD}${BLUE}   CRUD Operation Tests (Create Patterns & Anti-Patterns)${RESET}`);
	console.log(`${BOLD}${BLUE}═══════════════════════════════════════════════════════════════${RESET}\n`);
	...
	// PATTERN: Valid healthcheck with --host-header (no conflicts)
	{
		...
		const args = ["--name", "test-hc", "--type", "http", "--path", "/health", "--host-header", "example.com"];
		console.log(`   ${YELLOW}Args:${RESET} ${args.join(" ")}`);

		try {
			const parsed = parseCreationFlags(args, "healthcheck");
			console.log(`   ${CYAN}Parsed values:${RESET} ${JSON.stringify(Object.fromEntries(parsed.values), null, 2)}`);
			if (parsed.errors && parsed.errors.length > 0) {
				console.log(`   ${YELLOW}Parser errors:${RESET} ${parsed.errors.join(", ")}`);
			}
			const request = buildHealthcheckRequest(parsed, "default");
			console.log(`   ${CYAN}Request:${RESET} ${JSON.stringify(request, null, 2)}`);

			if (request.metadata?.name === "test-hc" && request.spec?.http_health_check?.host_header === "example.com") {
				console.log(`   ${GREEN}✅ Valid pattern - request built successfully${RESET}`);
				crudPassed++;
			} else {
				console.log(`   ${RED}❌ Pattern failed - unexpected request structure${RESET}`);
				console.log(`   ${RED}   Got: name=${request.metadata?.name}, host_header=${request.spec?.http_health_check?.host_header}${RESET}`);
				crudFailed++;
			}
		} catch (err) {
	...
	{
		const testName = "CRUD-HC-02: Valid HTTP healthcheck with --use-origin-server-name";
		console.log(`${BOLD}${testName}${RESET}`);
		const args = ["--name", "test-hc-2", "--type", "http", "--path", "/health", "--use-origin-server-name"];
		console.log(`   ${YELLOW}Args:${RESET} ${args.join(" ")}`);
		...
		try {
			...
			if (request.metadata?.name === "test-hc-2" && request.spec?.http_health_check?.use_origin_server_name !== undefined) {
				console.log(`   ${GREEN}✅ Valid pattern - request built successfully${RESET}`);
				console.log(`   ${CYAN}Request:${RESET} name=${request.metadata?.name}, use_origin_server_name=set`);
				crudPassed++;
			} else {
				console.log(`   ${RED}❌ Pattern failed - unexpected request structure${RESET}`);
				crudFailed++;
	...
	{
		const testName = "CRUD-HC-03: ANTI-PATTERN - Both --host-header and --use-origin-server-name";
		console.log(`${BOLD}${testName}${RESET}`);
		const args = ["--name", "test-hc-bad", "--type", "http", "--path", "/health", "--host-header", "example.com", "--use-origin-server-name"];
		console.log(`   ${YELLOW}Args:${RESET} ${args.join(" ")}`);
		...
		try {
			...
			const hasConflictError = parsed.errors?.some(e =>
			...
			if (hasConflictError) {
				console.log(`   ${GREEN}✅ Anti-pattern correctly detected - parser reported conflict${RESET}`);
				...
				const request = buildHealthcheckRequest(parsed, "default");
				const hasHostHeader = request.spec?.http_health_check?.host_header !== undefined;
				const hasUseOrigin = request.spec?.http_health_check?.use_origin_server_name !== undefined;

				if (hasHostHeader && hasUseOrigin) {
					console.log(`   ${YELLOW}⚠️  Warning: Both flags included in request (API will reject)${RESET}`);
					console.log(`   ${CYAN}Request contains:${RESET} host_header=${hasHostHeader}, use_origin_server_name=${hasUseOrigin}`);
					crudPassed++; // This is expected behavior - API will reject
				} else {
					console.log(`   ${GREEN}✅ Builder correctly chose one option over the other${RESET}`);
					crudPassed++;
				}
			}
		} catch (err) {
			// If the builder throws on conflict, that's also valid
			console.log(`   ${GREEN}✅ Anti-pattern correctly rejected: ${err}${RESET}`);
			crudPassed++;
		}
		console.log();
	}
	...
	// PATTERN: Valid origin_pool with --public-ip
	{
		...
		console.log(`${BOLD}${testName}${RESET}`);
		const args = ["--name", "test-pool", "--port", "443", "--public-ip", "1.2.3.4"];
		console.log(`   ${YELLOW}Args:${RESET} ${args.join(" ")}`);
		...
		try {
			...
			if (request.metadata?.name === "test-pool") {
				console.log(`   ${GREEN}✅ Valid pattern - request built successfully${RESET}`);
				console.log(`   ${CYAN}Request:${RESET} name=${request.metadata?.name}`);
				crudPassed++;
			} else {
				console.log(`   ${RED}❌ Pattern failed - unexpected request structure${RESET}`);
				crudFailed++;
	...
	{
		const testName = "CRUD-OP-02: Valid origin pool with multiple --public-ip (repeatable)";
		console.log(`${BOLD}${testName}${RESET}`);
		const args = ["--name", "test-pool-multi", "--port", "443", "--public-ip", "1.1.1.1", "--public-ip", "2.2.2.2"];
		console.log(`   ${YELLOW}Args:${RESET} ${args.join(" ")}`);
		...
		try {
			...
			const publicIps = origins.filter((o: any) => o.public_ip?.ip !== undefined);

			if (publicIps.length === 2) {
				console.log(`   ${GREEN}✅ Valid pattern - multiple origins created${RESET}`);
				console.log(`   ${CYAN}Origins:${RESET} ${publicIps.length} public IPs`);
				crudPassed++;
			} else {
				console.log(`   ${YELLOW}⚠️  Only ${publicIps.length} origins created (expected 2)${RESET}`);
	...
	{
		const testName = "CRUD-OP-03: ANTI-PATTERN - Both --public-ip and --public-name (conflict)";
		console.log(`${BOLD}${testName}${RESET}`);
		const args = ["--name", "test-pool-bad", "--port", "443", "--public-ip", "1.2.3.4", "--public-name", "example.com"];
		console.log(`   ${YELLOW}Args:${RESET} ${args.join(" ")}`);
		...
		try {
			...
			if (hasConflictError) {
				console.log(`   ${GREEN}✅ Anti-pattern correctly detected at parse time${RESET}`);
				console.log(`   ${CYAN}Error:${RESET} ${parsed.errors.join(", ")}`);
				crudPassed++;
			} else {
				...
				const request = buildOriginPoolRequest(parsed, "default");
				const origins = request.spec?.origin_servers || [];
				console.log(`   ${YELLOW}⚠️  Note: Parser allowed both types (${origins.length} origin(s) created)${RESET}`);
				console.log(`   ${CYAN}Behavior:${RESET} Each endpoint type creates a separate origin server`);
	...
	// Test TCP healthcheck (no HTTP-specific conflicts)
	{
		...
		try {
			...
			if (request.spec?.tcp_health_check !== undefined) {
				...
				console.log(`   ${CYAN}Type:${RESET} TCP (no HTTP conflict flags apply)`);
				crudPassed++;
			} else {
				console.log(`   ${RED}❌ Expected TCP healthcheck but got different type${RESET}`);
				console.log(`   ${CYAN}Request spec:${RESET} ${JSON.stringify(request.spec, null, 2)}`);
				crudFailed++;
			}
		} catch (err) {
			console.log(`   ${RED}❌ Error: ${err}${RESET}`);
