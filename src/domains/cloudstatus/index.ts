...
const statusCommand: CommandDefinition = {
  ...
  async execute(args, session): Promise<DomainCommandResult> {
    ...
    try {
      ...
      if (quiet) {
        const exitCode = statusIndicatorToExitCode(response.status.indicator);
...
const componentsCommand: CommandDefinition = {
  ...
  usage:
    "[--degraded-only] [--format <json|table|yaml|tsv>] [--spec] [--no-color]",
  ...
  async execute(args, session): Promise<DomainCommandResult> {
    ...
    const degradedOnly =
      filteredArgs.includes("--degraded-only") || filteredArgs.includes("-d");
    ...
    try {
      ...
      // Use unified formatter for json/yaml/tsv
      if (format === "json" || format === "yaml" || format === "tsv") {
        return successResult(formatListOutput(components, { format, noColor }));
      }
      ...
      for (const comp of components) {
        lines.push(`│ ${padEnd(comp.name, 39)} │ ${padEnd(comp.status, 18)} │`);
      }
...
const incidentsCommand: CommandDefinition = {
  ...
  usage:
    "[--active-only] [--format <json|table|yaml|tsv>] [--spec] [--no-color]",
  ...
  async execute(args, session): Promise<DomainCommandResult> {
    ...
    const activeOnly =
      filteredArgs.includes("--active-only") || filteredArgs.includes("-a");
    ...
    try {
      ...
      for (const inc of response.incidents) {
        const status = isIncidentActive(inc) ? "[ACTIVE]" : "[RESOLVED]";
...
function formatFullSummary(summary: SummaryResponse): string[] {
  ...
  const activeIncidents = summary.incidents.filter((i) => isIncidentActive(i));
