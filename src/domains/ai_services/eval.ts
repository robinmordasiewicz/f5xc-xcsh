...
export const evalQueryCommand: CommandDefinition = {
  ...
  usage:
    "<question> [--namespace <ns>] [--format <json|table|yaml|tsv>] [--spec] [--no-color]",
  aliases: ["q"],
  ...
  async execute(args, session): Promise<DomainCommandResult> {
    const { format, noColor, spec, namespace, question } = parseEvalQueryArgs(
      args,
      session,
    );
    ...
    if (!session.isTokenValidated()) {
      return errorResult("Not authenticated. Please check your API token.");
    }
...
export const evalFeedbackCommand: CommandDefinition = {
  ...
  usage:
    "--positive | --negative <type> [--comment <text>] [--query-id <id>] [--format <json|table|yaml|tsv>] [--spec] [--no-color]",
  aliases: ["fb"],
