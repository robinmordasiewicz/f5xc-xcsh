...
export class SubscriptionClient {
  ...
  async listAddonSubscriptions(
    ...
    const response = await this.apiClient.get<AddonSubscriptionListResponse>(
   ...
   */
  async unsubscribeFromAddon(request: AddonUnsubscribeRequest): Promise<void> {
    const response = await this.apiClient.post<Record<string, unknown>>(
   ...
   * Get monthly usage
   */
  async getMonthlyUsage(namespace: string = "system"): Promise<MonthlyUsage[]> {
  ...
  async getOverview(): Promise<SubscriptionOverview> {
    ...
    if (plan.status === "fulfilled") {
      ...
      errors.push(this.createAPIError("usage_plans/current", plan.reason));
    }
    ...
    if (currentUsage.status === "fulfilled") {
      ...
      errors.push(this.createAPIError("current_usage", currentUsage.reason));
    }
    ...
    if (paymentMethods.status === "fulfilled") {
      ...
      errors.push(
        this.createAPIError("billing/payment_methods", paymentMethods.reason),
      );
    }
