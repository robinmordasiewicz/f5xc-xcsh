  ...
  async execute(args, session): Promise<DomainCommandResult> {
    ...
    try {
      ...
      if (format === "json" || format === "yaml" || format === "tsv") {
        return successResult(formatDomainOutput(overview, { format, noColor }));
      }
      ...
      if (overview.addon_summary) {
        ...
        lines.push(`\u2502 Addon Services: ${padEnd(addonInfo, 38)} \u2502`);
      }
      ...
      if (overview.billing_status) {
        ...
        lines.push(`\u2502 Payment Status: ${padEnd(pmStatus, 38)} \u2502`);
      }
      ...
      if (overview.errors && overview.errors.length > 0) {
        ...
        for (const error of overview.errors) {
          ...
          const errorMsg = truncateMiddle(`${error.endpoint}${statusInfo}`, 52);
          lines.push(`\u2502   \u2022 ${padEnd(errorMsg, 50)} \u2502`);
        }
      }
      ...
      return errorResult(`Failed to get subscription overview: ${message}`);
    }
  },
};
...
const planShowCommand: CommandDefinition = {
  ...
  async execute(args, session): Promise<DomainCommandResult> {
    ...
    try {
      ...
      if (format === "json" || format === "yaml" || format === "tsv") {
        return successResult(formatDomainOutput(plan, { format, noColor }));
      }
...
const planListCommand: CommandDefinition = {
  ...
  async execute(args, session): Promise<DomainCommandResult> {
    ...
    try {
      ...
      if (format === "json" || format === "yaml" || format === "tsv") {
        return successResult(formatListOutput(plans, { format, noColor }));
      }
...
const addonListCommand: CommandDefinition = {
  ...
  usage:
    "[--subscribed] [--format <json|table|yaml|tsv>] [--spec] [--no-color]",
  ...
  async execute(args, session): Promise<DomainCommandResult> {
    ...
    const subscribedOnly =
      filteredArgs.includes("--subscribed") || filteredArgs.includes("-s");
    ...
    try {
      ...
      if (format === "json" || format === "yaml" || format === "tsv") {
        return successResult(formatListOutput(addons, { format, noColor }));
      }
...
const addonShowCommand: CommandDefinition = {
  ...
  async execute(args, session): Promise<DomainCommandResult> {
    ...
    try {
      ...
      if (format === "json" || format === "yaml" || format === "tsv") {
        return successResult(formatDomainOutput(addon, { format, noColor }));
      }
...
const addonStatusCommand: CommandDefinition = {
  ...
  async execute(args, session): Promise<DomainCommandResult> {
    ...
    try {
      ...
      if (format === "json" || format === "yaml" || format === "tsv") {
        return successResult(formatListOutput(statuses, { format, noColor }));
      }
      ...
      for (const status of statuses) {
        ...
        lines.push(`\u2502 ${name} \u2502 ${state} \u2502 ${activated} \u2502`);
      }
...
const quotaLimitsCommand: CommandDefinition = {
  ...
  async execute(args, session): Promise<DomainCommandResult> {
    ...
    try {
      ...
      if (format === "json" || format === "yaml" || format === "tsv") {
        return successResult(formatDomainOutput(response, { format, noColor }));
      }
...
const quotaUsageCommand: CommandDefinition = {
  ...
  async execute(args, session): Promise<DomainCommandResult> {
    ...
    try {
      ...
      if (format === "json" || format === "yaml" || format === "tsv") {
        return successResult(formatListOutput(usage, { format, noColor }));
      }
...
const usageCurrentCommand: CommandDefinition = {
  ...
  async execute(args, session): Promise<DomainCommandResult> {
    ...
    try {
      ...
      if (format === "json" || format === "yaml" || format === "tsv") {
        return successResult(formatDomainOutput(usage, { format, noColor }));
      }
      ...
      if (usage.usage_items && usage.usage_items.length > 0) {
        ...
        for (const item of usage.usage_items) {
          const cost = formatCurrency(item.total_cost, usage.currency);
          lines.push(
...
const usageMonthlyCommand: CommandDefinition = {
  ...
  async execute(args, session): Promise<DomainCommandResult> {
    ...
    const limitValue = limitIdx !== -1 ? filteredArgs[limitIdx + 1] : undefined;
    if (limitValue) {
    ...
    try {
      ...
      if (format === "json" || format === "yaml" || format === "tsv") {
        return successResult(formatListOutput(months, { format, noColor }));
      }
      ...
      for (const month of months) {
        ...
        lines.push(`\u2502 ${monthStr} \u2502 ${cost} \u2502 ${status} \u2502`);
      }
...
const billingPaymentListCommand: CommandDefinition = {
  ...
  async execute(args, session): Promise<DomainCommandResult> {
    ...
    try {
      ...
      if (format === "json" || format === "yaml" || format === "tsv") {
        return successResult(formatListOutput(methods, { format, noColor }));
      }
...
const billingInvoiceListCommand: CommandDefinition = {
  ...
  async execute(args, session): Promise<DomainCommandResult> {
    ...
    try {
      ...
      if (format === "json" || format === "yaml" || format === "tsv") {
        return successResult(formatListOutput(invoices, { format, noColor }));
      }
      ...
      for (const invoice of invoices) {
        const num = padEnd(invoice.invoice_number ?? invoice.invoice_id, 14);
    ...
    try {
      ...
      if (
        ...
        report.billing = {
          ...
          recent_invoices:
            invoices.status === "fulfilled" ? invoices.value.slice(0, 3) : [],
        };
      }
      ...
      if (format === "json" || format === "yaml" || format === "tsv") {
        return successResult(formatDomainOutput(report, { format, noColor }));
      }
      ...
      if (quotaUsage.status === "fulfilled") {
        ...
        const avgUtil =
          usage.length > 0
            ? usage.reduce((sum, q) => sum + (q.percentage ?? 0), 0) /
              usage.length
            : 0;
        lines.push(
