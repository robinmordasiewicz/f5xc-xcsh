...
function flattenSpans(
  ...
  for (let i = 0; i < spans.length; i++) {
    ...
    if (span.children.length > 0) {
      result.push(...flattenSpans(span.children, depth + 1, bottleneckIds));
    }
  }
...
export function renderWaterfall(report: ProfileReport): string {
  ...
  for (const span of flatSpans) {
    ...
    const bar = renderBar(span.startTime, span.duration, totalTime, barWidth);
  ...
  // Legend
  lines.push("[N] = Network call  !! = Bottleneck (>500ms)  ! = Slow (>100ms)");
