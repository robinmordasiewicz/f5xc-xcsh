...
export class HeadlessController {
    ...
    switch (input.type) {
      case "command":
        if (input.value !== undefined) {
          ...
          this.write(
            createErrorMessage('Missing "value" for command input', 1),
          );
          this.sendPrompt();
        }
        break;
      ...
      default:
        this.write(createErrorMessage(`Unknown input type: ${input.type}`, 1));
        this.sendPrompt();
    }
  }
  ...
  async run(): Promise<void> {
    ...
    for await (const line of this.rl) {
      ...
      if (!input) {
        this.write(createErrorMessage(`Invalid JSON input: ${trimmed}`, 1));
        this.sendPrompt();
        continue;
      }
