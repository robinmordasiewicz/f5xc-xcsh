...
export function buildAIServicesSpecs(): Record<string, CommandSpec> {
  return {
    query: buildCommandSpec({
      ...
      usage:
        "xcsh ai_services query <question> [--namespace <ns>] [--output <format>]",
      flags: [
        {
          name: "--namespace",
          alias: "-ns",
    ...
    feedback: buildCommandSpec({
      ...
      usage:
        "xcsh ai_services feedback [--positive | --negative <type>] [--comment <text>] [--query-id <id>]",
      flags: [
        {
          name: "--positive",
    ...
    if (parts.length >= 2 && parts[parts.length - 1] === "create") {
      ...
      if (resourceSpec) {
        ...
        return {
          ...
          examples: buildResourceExamples(resourceType, aiAssistantGuide),
