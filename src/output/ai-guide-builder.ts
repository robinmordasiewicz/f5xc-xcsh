...
export function buildValidationRules(
	...
	const dependencies = fields
		.filter(
			(f) =>
				f.extensions.conflictsWith &&
				f.extensions.conflictsWith.length > 0,
		)
		.map((f) => ({
			field: f.name,
...
export function buildTroubleshooting(resourceType: string): Troubleshooting[] {
	const common: Troubleshooting[] = [
		...
		},
		{
			...
			solution:
				"Choose a unique name or delete the existing resource first",
			relatedFields: ["metadata.name", "metadata.namespace"],
		},
	];
		...
		case "healthcheck":
			return [
				...
				},
				{
					...
					solution:
						"Check expected status codes and paths match your backend",
					relatedFields: [
