  ...
  if (Array.isArray(value)) {
    ...
    for (let i = 0; i < value.length; i++) {
      ...
      if (typeof item === "object" && item !== null) {
        ...
        const formatted = formatIndentedTree(item, indent, maxDepth, maxWidth);
  ...
  if (typeof value === "object") {
    ...
    for (const key of keys) {
      ...
      if (typeof childValue === "object" && childValue !== null) {
        const childObj = childValue as Record<string, unknown> | unknown[];
        ...
        if (isEmpty) {
          ...
          lines.push(`${key}: ${Array.isArray(childObj) ? "[]" : "{}"}`);
        } else {
...
export function formatBeautifulTable(
  ...
  const widths = calculateColumnWidths(config.columns, data, effectiveMaxWidth);
  ...
  if (config.title) {
    ...
    lines.push(
      buildHorizontalLine(box.topLeft, box.topT, box.topRight, box.horizontal),
    );
  }
...
export function formatResourceDetails(
  ...
  const metadata = data.metadata as Record<string, unknown> | undefined;
  const name =
    (metadata?.name as string) || (data.name as string) || "Resource Details";
