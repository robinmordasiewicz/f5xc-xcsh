...
export function App({ initialSession }: AppProps = {}): React.ReactElement {
  ...
  const [pendingRawStdout, setPendingRawStdout] = useState<string | null>(null);
  ...
  const addOutput = useCallback((line: string) => {
    ...
    setOutputItems((prev) => {
      ...
      const uniqueCommands = new Set(combined.map((item) => item.commandId));
  ...
  const applyCompletion = useCallback(
    (suggestion: Suggestion) => {
      ...
      if (words.length === 0 || currentInput === "") {
        ...
        const hasEscapePrefix = words.length === 1 && lastWord.startsWith("/");
        const matchWord = hasEscapePrefix ? lastWord.slice(1) : lastWord;

        if (
          matchWord === "" ||
          suggestion.value.toLowerCase().startsWith(matchWord.toLowerCase())
        ) {
    ...
    async (cmd: string) => {
      ...
      if (result.rawStdout) {
        ...
        debugOutput("Processing output lines", {
          ...
          lastLine: result.output[result.output.length - 1]?.slice(0, 100),
        });
  ...
  useInput((char, key) => {
    ...
    if (key.tab) {
      ...
      if (completion.isShowing) {
        ...
        const selected = completion.suggestions.at(completion.selectedIndex);
        if (selected) {
          applyCompletion({
            label: selected.text,
            value: selected.text,
  ...
  useEffect(() => {
    const uniqueCommands = new Set(outputItems.map((item) => item.commandId));
          ...
          <>
            ...
            {completion.isShowing && completion.suggestions.length > 0 ? (
              ...
              <StatusBar
                ...
                hint={statusHint}
                connectionStatus={
                  inactivity.isSleeping ? "unknown" : healthCheck.health.status
                }
