...
export function ChatMode({
  ...
  const showHelp = useCallback(() => {
    ...
    addMessage("system", "  /clear, /c          - Clear conversation context");
    addMessage(
      "system",
      "  /feedback <type>    - Submit feedback (positive/negative)",
    );
    addMessage("system", "  1, 2, 3...          - Select a follow-up question");
    addMessage("system", "");
    addMessage("system", "Just type your question to query the AI assistant.");
    addMessage("system", "");
  }, [addMessage]);
  ...
  const handleFeedback = useCallback(
    async (feedbackInput: string) => {
      ...
      if (!state.lastQueryId || !state.lastQuery) {
        addMessage("system", "No previous query to provide feedback for.");
        return;
      }
      ...
      try {
        ...
        if (feedbackType === "positive" || feedbackType === "+") {
          ...
          addMessage("system", "Positive feedback submitted. Thank you!");
