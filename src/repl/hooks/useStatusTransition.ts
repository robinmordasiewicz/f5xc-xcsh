  ...
  useEffect(() => {
    ...
    if (isChecking) {
      ...
      const updateCheckingProgress = () => {
        ...
        const fadeFraction = Math.min(elapsed / QUICK_FADE_DURATION_MS, 1);
        const newProgress = startProgress + remainingProgress * fadeFraction;
      ...
      const intervalId = setInterval(updateCheckingProgress, updateInterval);
