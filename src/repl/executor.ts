...
async function handleDirectNavigation(
  ...
  if (merged?.hasExtension && !merged.hasGeneratedDomain) {
    ...
    if (!cmd.targetAction) {
      ...
      for (const [name, cmdDef] of merged.extensionCommands) {
        const aliases = cmdDef.aliases ? ` (${cmdDef.aliases.join(", ")})` : "";
    ...
    return {
      output: [
        ...
        ...Array.from(merged.extensionCommands.keys()).map((c) => `  ${c}`),
      ],
      shouldExit: false,
      shouldClear: false,
      contextChanged: false,
  ...
  if (cmd.targetAction === "help") {
    ...
    const { resourceType } = parseCommandArgs(cmd.args, domainResourceTypes);
...
async function executeAPICommand(
  ...
  if (effectiveNamespace) {
    ...
    if (!nsNameValidation.valid) {
      const errorMsg = nsNameValidation.message ?? "Invalid namespace name";
      ...
      return {
        output: [`ERROR: Invalid namespace name '${effectiveNamespace}'`, tip],
        shouldExit: false,
        shouldClear: false,
        contextChanged: false,
        error: errorMsg,
  ...
  try {
    ...
    switch (action) {
      ...
      case "get": {
        if (!name) {
          return {
            output: ["Error: Resource name required for 'get' action"],
            shouldExit: false,
            shouldClear: false,
            contextChanged: false,
      ...
      case "delete": {
        if (!name) {
          return {
            output: ["Error: Resource name required for 'delete' action"],
            shouldExit: false,
            shouldClear: false,
            contextChanged: false,
        ...
        if (dryRun) {
          ...
          if (preDeleteResult.exists) {
            ...
            if (checkDeps && preDeleteResult.dependencies) {
              dryRunOutput.push("Dependencies that would be affected:");
        ...
        if (!preDeleteResult.exists) {
          const resourceTypeName = effectiveResourceType || canonicalDomain;
          return {
            output: [
      ...
      case "apply": {
        ...
        if (
          ...
          const parsed = parseCreationFlags(args, effectiveResourceType);
        ...
        // Neither --file nor creation flags provided
          ...
          const supportsFlagCreation =
            effectiveResourceType && hasResourceBuilder(effectiveResourceType);
          const hints = [
        ...
        if (action === "create" && effectiveResourceType) {
          ...
          const forceFlag = args.some((a) => a === "--force" || a === "-f");
          const quotaResult = await checkQuotaBeforeCreate(subscriptionClient, {
          ...
          if (quotaResult.level !== "none") {
            const quotaWarningLines = formatQuotaWarning(quotaResult);
        ...
        if (action === "create") {
          ...
          const skipConfirmFlag = args.some(
            (a) => a === "--yes" || a === "-y" || a === "--force" || a === "-f",
          );

          // If no skip flag, enter confirmation mode
          if (!skipConfirmFlag && requestBody) {
            const resourceName =
              (requestBody.metadata as Record<string, unknown>)?.name ??
              name ??
              "";
            const effectiveNs =
              (requestBody.metadata as Record<string, unknown>)?.namespace ??
              effectiveNamespace ??
              "";
          ...
          const resourceName =
            (requestBody.metadata as Record<string, unknown>)?.name ?? name;
          ...
          try {
            ...
            if (postError instanceof APIError && postError.statusCode === 409) {
              ...
              const response = await client.put(putPath, requestBody);
              result = response.data ?? {
      ...
      case "status": {
        if (!name) {
          return {
            output: ["Error: Resource name required for 'status' action"],
            shouldExit: false,
            shouldClear: false,
            contextChanged: false,
        ...
        if (!specResourceType) {
          ...
          return {
            output: [
              ...
              "",
              "Available resources:",
              ...primaryResources.map((r) => `  - ${r.name}: ${r.description}`),
            ],
            shouldExit: false,
            shouldClear: false,
            contextChanged: false,
...
export function getCommandSuggestions(
  ...
  if (ctx.isDomain() && !ctx.isAction()) {
    ...
    for (const cmd of extCmds) {
      if (!input || cmd.toLowerCase().startsWith(input.toLowerCase())) {
        const cmdDef = extensionRegistry.getExtensionCommand(domain, cmd);
        suggestions.push({
          text: cmd,
