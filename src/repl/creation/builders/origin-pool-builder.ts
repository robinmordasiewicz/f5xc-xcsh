...
export function buildOriginPoolRequest(
	...
	if (outlierDetection === "disable") {
		...
		if (consecutive5xx !== undefined)
			odConfig.consecutive_5xx = consecutive5xx;
		...
		const baseEjectionTime = getFlagIntValue(
			flags,
...
export function validateOriginPoolFlags(
	...
	if (
		...
		errors.push(
			"--od-consecutive-gateway-failure must be between 0 and 1024",
		);
	}
	...
	if (subsetLb !== "enable") {
		...
		if (subsetFallback) {
			errors.push(
				"--subset-default-fallback requires --subset-lb=enable",
			);
		}
	}
	...
	if (healthCheckPortSource !== "custom") {
		const healthCheckPortValue = getFlagIntValue(
			flags,
			"--health-check-port",
		);
