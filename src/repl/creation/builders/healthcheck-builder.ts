...
export function buildHealthcheckRequest(
  ...
  switch (type) {
    case "http": {
      ...
      for (const headerStr of customHeaders) {
        ...
        if (key.length === 0 || key.length > 256) {
          throw new Error(`Header key length must be 1-256 chars: ${key}`);
        }
        if (value.length === 0 || value.length > 2048) {
          throw new Error(`Header value length must be 1-2048 chars: ${value}`);
        }

        // Check for duplicate keys
        if (headersObj[key]) {
          console.warn(`Duplicate header key '${key}'. Using last value.`);
        }
      ...
      if (requestHeadersToRemove.length > 0) {
        ...
        for (const header of requestHeadersToRemove) {
          if (header.length > 256) {
            throw new Error(`Header name too long (max 256 chars): ${header}`);
          }
        }
    ...
    case "tcp": {
      ...
      if (sendPayload) {
        try {
          request.spec.tcp_health_check.send_payload = toApiHex(sendPayload);
        } catch (error) {
          throw new Error(
...
export function validateHealthcheckFlags(
  ...
  if (type === "http") {
    ...
    if (jitterPercent !== undefined) {
      if (jitterPercent !== 0 && (jitterPercent < 10 || jitterPercent > 50)) {
        errors.push("--jitter-percent must be 0 or between 10 and 50");
      }
    }

    // Validate expected status codes format
    const expectedStatusCodes = getFlagValues(flags, "--expected-status-codes");
