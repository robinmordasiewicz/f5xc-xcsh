...
export function parseCreationFlags(
  ...
  while (i < args.length) {
    ...
    if (def.isRepeatable && def.maxOccurrences) {
      ...
      result.errors.push(`Flag ${def.name} cannot be specified multiple times`);
      i++;
      continue;
    }
    ...
    if (def.valueType === "integer") {
      ...
      if (isNaN(num)) {
        result.errors.push(`Invalid integer value '${value}' for ${def.name}`);
        continue;
      }
    }
    ...
    if (def.conflictsWith && result.values.has(def.name)) {
      for (const conflictingFlag of def.conflictsWith) {
        if (result.values.has(conflictingFlag)) {
          ...
          const conflictKey = [def.name, conflictingFlag].sort().join("|");
